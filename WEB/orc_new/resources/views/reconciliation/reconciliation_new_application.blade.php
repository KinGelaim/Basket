...
@section('content')
	<div class="flex-center position-ref full-height">
		@if (Auth::User())
			<div class="container">
				<form id='post_send' method='POST' action='{{route("new_applications.reconciliation_store", $new_application->id)}}'>
					{{csrf_field()}}
					<div class='row'>
						<div class="col-md-2">
						</div>
						<div class="col-md-4">
							<button class='btn btn-primary btn-href' type='button' href='{{route("new_applications.show", $new_application->id)}}'>Перейти к заявке</button>
						</div>
					</div>
					<div class='row'>
						<div class="col-md-2">
					...
					<div class='row'>
						...
						<div class="col-md-8">
							<input class='form-control' type='text' value='{{$new_application->name_counterpartie_contract}}' readonly />
						</div>
					</div>
					<div class='row'>
						<div class="col-md-2">
						...
						<div class="col-md-8">
							<input class='form-control' type='text' value='{{$new_application->item_new_application}}' readonly />
						</div>
					</div>
					<div class='row'>
						<div class="col-md-2">
						...
						<div class="col-md-8">
							<input class='form-control' type='text' value='{{$new_application->name_work_new_application}}' readonly />
						</div>
					</div>
					<div class='row'>
						<div class="col-md-2">
			...
			<div class="modal fade" id="new_resolution" tabindex="-1" role="dialog" aria-labelledby="newResolutionModalLabel" aria-hidden="true" attr-open-modal='{{$errors->has("number_application_update") || $errors->has("number_outgoing_update") || $errors->has("date_outgoing_update") || $errors->has("number_incoming_update") || $errors->has("date_incoming_update") || $errors->has("directed_application_update") || $errors->has("date_directed_update") || Session::has("new_scan") ? print("open") : print("")}}'>
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						<form method='POST' file='true' enctype='multipart/form-data' action='{{route("resolution_store",$new_application->id)}}'>
							{{csrf_field()}}
							<div class="modal-header">
