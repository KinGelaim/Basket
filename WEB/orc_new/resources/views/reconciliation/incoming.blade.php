								...
								<tbody>
									@foreach($applications as $application)
										<tr class='rowsContract' id_application='{{$application->id}}' >
											...
											@if(Auth::User()->hasRole()->role != 'Администрация')
												<td><button type='button' class='btn btn-primary btn-href' type='button' href='{{route("department.reconciliation.create_document",$application->id)}}' {{ $application->id_contract_application != null || $application->id_document_application != null || $application->id_application_document != null  ? 'disabled' : ''}}>Создать</button></td>
											@endif
										</tr>
									@endforeach
								</tbody>
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="messageModalLabel">Письмо</h5>
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть письмо</button>
							</div>
						</div>
					</div>
				</div>
