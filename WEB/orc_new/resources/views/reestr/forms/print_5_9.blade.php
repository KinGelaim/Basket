						...
						<table class="table table-bordered tablePrint" style='margin: 0 auto; margin-top:20px; margin-bottom: 10px;'>
							<thead style='text-align: center;'>
								<tr style='border: solid 1px black;'>
									<th>№ п/п</th>
									<th>Номер Дог./Контр. (Контрагента/наш), дата</th>
									<th>Контрагент</th>
									<th>Предмет Дог./Контр.</th>
									<th>Сумма по Дог./Контр.</th>
									<th>Исполнитель</th>
									<th>Договорная документация (прот. разногл., прот. согл. разногл., доп. согл. и др.)</th>
								</tr>
							</thead>
							<tbody>
								...
								@foreach($department as $contract)
									<tr style='border: solid 1px black;'>
										<td>{{$k++}}</td>
										<td>{{$contract->number_counterpartie_contract_reestr ? $contract->number_counterpartie_contract_reestr . '/' : ''}}{{$contract->number_contract}} {{$contract->date_contract_on_first_reestr ? 'от ' . $contract->date_contract_on_first_reestr : ''}}</td>
