<?php
	...
	if(isset($_POST['index_book'])){
		...
		if(strlen($_POST['type']) > 0){
			...
			$search_index = mysqli_query($link, "SELECT inventory_number FROM books WHERE inventory_number=" . $inventory_number . ' AND type=' . $type);
			$count = 0;
			...
			if($count > 0){
				$result = array(
					'result' => 'error',
					'message' => 'Добавить не удалось! Такой номер по описи уже существует!'
				);
			...
			if ($result = mysqli_query($link, "INSERT INTO books (type,inventory_number,index_book,title_book,date_begin_end,count,comment,place_s,place_p,department,date_incoming_outgoing,counterpartie,document_incoming,fond) VALUES (" . $type . "," . $inventory_number . "," . $index_book . "," . $title_book . "," . $date_begin_end . "," . $count . "," . $comment . "," . $place_s . "," . $place_p . "," . $department . "," . $date_incoming_outgoing . "," . $counterpartie . "," . $document_incoming . "," . $fond . ")")) {
				$per_result = 'success';
			}
