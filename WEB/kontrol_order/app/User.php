...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'surname', 'name', 'patronymic', 'id_role', 'position_department', 'telephone', 'login', 'password',
    ];
	...
	{
		return DB::SELECT('SELECT role FROM roles WHERE id=:id',['id'=>$this->id_role])[0];
	}
	...
	public static function getAllShortFIO()
	{
		$users = User::select(['id','surname','name','patronymic'])->orderBy('surname','asc')->get();
		foreach($users as $user)
		{
			$user->shortFIO = $user->surname . ' ' . $user->name[0] . '.' . $user->patronymic[0] . '.';
			dd($user->shortFIO);
		}
		return $users;
	}
