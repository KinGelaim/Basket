...
function initTexture() {
	...
	doorExitTexture = gl.createTexture();
	doorExitTexture.image = new Image();
	doorExitTexture.image.onload = function () {
		handleLoadedTexture(doorExitTexture)
	}

	doorExitTexture.image.src = "images/DoorExit.jpg";
}
...
function loadWorld() {
	...
	request.open("GET", "world/first_world.txt");
...
var worldDoorExitVertexPositionBuffer = null;
var worldDoorExitVertexTextureCoordBuffer = null;
...
function handleLoadedWorld(data) {
	...
	var vertexDoorExitCount = 0;
	var vertexDoorExitPositions = [];
	var vertexDoorExitTextureCoords = [];
	for (var i in lines) {
		...
		if (vals.length == 6 && vals[0] != "//") {
			...
			if(vals[0] == "dex") {
				vertexDoorExitPositions.push(parseFloat(vals[1]));
				vertexDoorExitPositions.push(parseFloat(vals[2]));
				vertexDoorExitPositions.push(parseFloat(vals[3]));

				vertexDoorExitTextureCoords.push(parseFloat(vals[4]));
				vertexDoorExitTextureCoords.push(parseFloat(vals[5]));

				vertexDoorExitCount += 1;
			}
		}
	}
	...
	// Дверь Exit
	worldDoorExitVertexPositionBuffer = gl.createBuffer();
	gl.bindBuffer(gl.ARRAY_BUFFER, worldDoorExitVertexPositionBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vertexDoorExitPositions), gl.STATIC_DRAW);
	worldDoorExitVertexPositionBuffer.itemSize = 3;
	worldDoorExitVertexPositionBuffer.numItems = vertexDoorExitCount;

	worldDoorExitVertexTextureCoordBuffer = gl.createBuffer();
	gl.bindBuffer(gl.ARRAY_BUFFER, worldDoorExitVertexTextureCoordBuffer);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vertexDoorExitTextureCoords), gl.STATIC_DRAW);
	worldDoorExitVertexTextureCoordBuffer.itemSize = 2;
	worldDoorExitVertexTextureCoordBuffer.numItems = vertexDoorExitCount;
...
function drawScene() {
	...
	// DoorExit
	gl.activeTexture(gl.TEXTURE0);
	gl.bindTexture(gl.TEXTURE_2D, doorExitTexture);
	gl.uniform1i(shaderProgram.samplerUniform, 0);

	gl.bindBuffer(gl.ARRAY_BUFFER, worldDoorExitVertexTextureCoordBuffer);
	gl.vertexAttribPointer(shaderProgram.textureCoordAttribute, worldDoorExitVertexTextureCoordBuffer.itemSize, gl.FLOAT, false, 0, 0);

	gl.bindBuffer(gl.ARRAY_BUFFER, worldDoorExitVertexPositionBuffer);
	gl.vertexAttribPointer(shaderProgram.vertexPositionAttribute, worldDoorExitVertexPositionBuffer.itemSize, gl.FLOAT, false, 0, 0);

	setMatrixUniforms();
	gl.drawArrays(gl.TRIANGLES, 0, worldDoorExitVertexPositionBuffer.numItems);
}
...
// ---------Проверка дверей---------
var isVisibleEnter22 = false;
var isVisibleEnterIT = false;
var isVisibleEnterExit = false;
function checkDoor() {
	if(xPos < -8 && xPos > -9.5 && zPos < -2.2 && zPos > -4 && !isVisibleEnter22)
	{
		isVisibleEnter22 = true;
		document.getElementById("enter22").style['display'] = 'block';
		console.log(22);
	}
	if(isVisibleEnter22)
		if(xPos > -8 || xPos < -9.5 || zPos > -2.2 || zPos < -4)
		{
			isVisibleEnter22 = false;
			document.getElementById("enter22").style['display'] = 'none';
			console.log('exit22');
		}
	if(xPos < -9.5 && xPos > -10.8 && zPos < -2.2 && zPos > -4 && !isVisibleEnterIT)
	{
		isVisibleEnterIT = true;
		document.getElementById("enterIT").style['display'] = 'block';
		console.log('IT');
	}
	if(isVisibleEnterIT)
		if(xPos > -9.5 || xPos < -10.8 || zPos > -2.2 || zPos < -4)
		{
			isVisibleEnterIT = false;
			document.getElementById("enterIT").style['display'] = 'none';
			console.log('exitIT');
		}
	if(xPos < 0.5 && xPos > -0.5 && zPos < 0.5 && zPos > -2 && !isVisibleEnterExit)
	{
		isVisibleEnterExit = true;
		document.getElementById("enterExit").style['display'] = 'block';
		console.log('exit');
	}
	if(isVisibleEnterExit)
		if(xPos > 0.5 || xPos < -0.5 || zPos > 0.5 || zPos < -2)
		{
			isVisibleEnterExit = false;
			document.getElementById("enterExit").style['display'] = 'none';
...
function handleMouseDown() {
	console.log("x:" + xPos + " z:" + zPos + " yaw:" + yaw);
}